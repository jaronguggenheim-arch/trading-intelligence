// api/push.js — Web Push subscription management + VAPID push delivery
// Uses globalThis.crypto.subtle (Node 18 Web Crypto — no imports needed)
//
...
//
// Env vars: VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY, VAPID_EMAIL, KV_REST_API_URL, KV_REST_API_TOKEN
...
async function makeVapidJwt(audience, subject, privKeyB64u) {
  ...
  const privKey = await globalThis.crypto.subtle.importKey(
    'pkcs8',
    buildPkcs8(b64uDecode(privKeyB64u)),
...
// Build a minimal PKCS8 DER wrapper around a raw 32-byte P-256 private key
function buildPkcs8(rawKey) {
  // RFC 5958 PKCS#8 structure for EC P-256 (no public key included)
...
// ── RFC 8291 push encryption (aesgcm) ─────────────────────────────────────────
async function sendPush(subscription, payloadStr, pubKeyB64u, privKeyB64u, vapidEmail) {
  ...
  if (!clientPubRaw.length || !authSecret.length) {
    // No encryption keys — send unencrypted (will likely be rejected by browser)
  ...
  // Generate ephemeral server ECDH key
  const serverKP = await globalThis.crypto.subtle.generateKey(
    { name: 'ECDH', namedCurve: 'P-256' }, true, ['deriveBits']
  );
  const serverPubRaw = new Uint8Array(
    await globalThis.crypto.subtle.exportKey('raw', serverKP.publicKey)
  );
  ...
  // PRK = HMAC-SHA256(authSecret, sharedSecret)
  ...
  // context = 'P-256\0' + uint16(65) + clientPubRaw + uint16(65) + serverPubRaw
  const ctx = concat(
    new TextEncoder().encode('P-256\0'),
    new Uint8Array([0, 65]), clientPubRaw,
    new Uint8Array([0, 65]), serverPubRaw
  );

  // HKDF expand (one-step, single block)
  const hkdfExpandKey = await globalThis.crypto.subtle.importKey(
    'raw', prk, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']
  );

  const salt = globalThis.crypto.getRandomValues(new Uint8Array(16));
  // saltedPRK = HMAC-SHA256(salt, prk)
  const saltKey = await globalThis.crypto.subtle.importKey(
    'raw', salt, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']
  );
  const saltedPrk = new Uint8Array(await globalThis.crypto.subtle.sign('HMAC', saltKey, prk));
  const saltedKey = await globalThis.crypto.subtle.importKey(
    'raw', saltedPrk, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']
  );
    ...
    await Promise.all(emails.map(async (email) => {
      ...
      for (const sub of subs) {
        try {
          ...
          else { if (status === 410) await kvSet(kvUrl, kvToken, PUSH_KEY(email), subs.filter(s => s.endpoint !== sub.endpoint)); failed++; }
        } catch { failed++; }
      }
    }));
